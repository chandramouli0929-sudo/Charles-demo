...
def render_input_phase():
    ...
    st.markdown('<div class="hero-title">⚙️ charles-agentic-ai-demo</div>', unsafe_allow_html=True)
    st.markdown(
...
def render_analyzing_phase():
    ...
    with st.spinner("🤖 charles-agentic-ai-demo is analyzing your requirement..."):
        try:
...
**charles-agentic-ai-demo Assessment:**
...
* **📂 Workspace Directory on Disk:** `{ws_path}`
    ...
    with tab2:
        ...
        st.caption("Test the live FastAPI application generated by charles-agentic-ai-demo directly from this UI!")
        ...
        if test_redirect_btn:
            try:
                ...
                req = urllib.request.Request(redirect_url, headers={"User-Agent": "charles-agentic-ai-demo-Playground/1.0"})
